...
public class NPCBuildFirewall : MonoBehaviour 
{
	public int playerID; 
	...
	void Update()
	{
		currentLane = npcMovement.currentLane;
		...
		if (Input.GetKeyDown (KeyCode.Space) && playerID == 0) 
		{
			if(sceneManagerScript.p1totalFirewalls < MAX_FIREWALLS)
			{
				...
				firewallControllerScript = npcMovement.deskArray [currentLane].GetComponent<FirewallController> (); 

				// Print the current lane value and the player ID value
				Debug.Log ("PLAYER ID: " + playerID + "CURRENT LANE: " + npcMovement.currentLane);
